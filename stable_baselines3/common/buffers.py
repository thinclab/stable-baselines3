...
class RolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        if self.obs_shape==(1,):
            self.observations = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
    ...
    def add(
        ...
        # print("pre reshape obs.shape",obs.shape)

        # (commented because shape is already adjusted) Reshape needed when using multiple envs with discrete observations
        # as numpy cannot broadcast (n_discrete,) to (n_discrete, 1)
        # if isinstance(self.observation_space, spaces.Discrete):
        #     obs = obs.reshape((self.n_envs,) + self.obs_shape)
        #     print("post reshape obs.shape",obs.shape)
