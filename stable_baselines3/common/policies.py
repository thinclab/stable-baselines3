...
class ActorCriticPolicy(BasePolicy):
    ...
    def prob_acts(self, obs: th.Tensor, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        Evaluate action likelihoods according to the current policy, given the observations.
        ...
        latent_pi, latent_vf, latent_sde = self._get_latent(obs) 
        distribution = self._get_action_dist_from_latent(latent_pi, latent_sde) 
        prob_all_acts = distribution.distribution.probs
        log_prob = distribution.log_prob(actions) 
        probs = th.exp(log_prob) 
        return probs
